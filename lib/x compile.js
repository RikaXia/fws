...
inPath['jsx'] = path.join(fws.srcPath,'tsx');
outPath['jsx'] = path.join(fws.devPath,'js');
...
let compile = (filePath)=>{
    ...
    if(pt.type === 'file'){
        if(fileType === '.pug' && !isFileSlur('pug',filePath) && notIgnoreDir){
            ...
            //tsx输出于跟当前相同目录            
            dist = path.join(path.dirname(filePath),fileName+'.jsx');
            ts2js(filePath,dist);
        }else if(fileType === '.jsx' && !isFileSlur('jsx',filePath) && notIgnoreDir){
            setDist('jsx','js');
            new Jsx2js({
                src:filePath,
                dist:dist,
            ...
            if(!isDataFile(filePath) && notDir_(filePath) && fileType !== '.map'){
                dist = filePath.replace(fws.srcPath,fws.devPath);
